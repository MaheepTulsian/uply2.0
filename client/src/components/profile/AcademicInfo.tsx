...
function AcademicInfo({ formRef, onSuccess }: AcademicInfoProps) {
    ...
    useEffect(() => {
        const fetchAcademicInfo = async () => {
            ...
            try {
                ...
                setFetchError(
                    error.response?.data?.error || "Failed to fetch academic information"
                );
            } finally {
                setIsLoading(false);
            }
    ...
    const onSubmit: SubmitHandler<AcademicFormData> = async (data) => {
        try {
            const response = await axios.post(
                `${API_BASE_URL}/${id}/academic_info`,
            ...
            const errorMessage =
                error.response?.data?.error || "Failed to update academic information";
                ...
                <div className="space-y-4 text-lg">
                    ...
                    {currentAcademics.map((entry, index) => (
                        <div key={index} className="border p-3 rounded-lg relative">
                            <p>
                                <strong>Institution:</strong> {entry.institution}
                            </p>
                            <p>
                                <strong>Degree:</strong> {entry.degree}
                            </p>
                            <p>
                                <strong>Field of Study:</strong> {entry.fieldOfStudy}
                            </p>
                            <p>
                                <strong>Start Date:</strong> {entry.startDate}
                            </p>
                            <p>
                                <strong>End Date:</strong> {entry.endDate || "Present"}
                            </p>
                            <p>
                            ...
                            >
                                ...
                                {[
                                    ...
                                    <FormField
                                        ...
                                        name={`academics.${index}.${fieldName}` as const}
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>
                                                    {fieldName
